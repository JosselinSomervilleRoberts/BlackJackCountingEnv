...
if __name__ == "__main__":
    N_EPISODES = 5000000
    def learning_rate(n):
        return 0.001 + 0.2 * (max(0, N_EPISODES - n) / N_EPISODES) ** 2
    def epsilon_func(n):
        return 0.5 * (max(0, 0.5*N_EPISODES - n) / N_EPISODES) ** 2

    rules = {"double_on": 2, "split": True, "surrender": False, "dealer_hit_soft_17": True, "resplit": True, "blackjack_payout": 1.5, "floor_finished_reward": True}
    decks = DecksOfCards(nb_decks=6, fraction_not_in_play=0.2)
    env = BlackJackPlayingEnv(decks = decks, rules=rules)
    agent = QLearningAgent(rules, epsilon_func=epsilon_func, delta = 0, discount_factor=0.9, learning_rate=learning_rate)
    ...
    for i in tqdm(range(N_EPISODES)):
        obs_composed = env.reset()
        (obs, reward, done, info) = obs_composed

        if done: # the player or the dealer has a natural blackjack
        ...
        while not done:
            ...
            action = agent.get_legal_action(obs, learning=True)
            obs_composed, reward, done, info = env.step(action)
            # print(obs_composed)
            (obs, reward, done2, info) = obs_composed
            #agent.Qlearn(prev_obs, action, obs, reward, done)
            data.append({"state": prev_obs, "action": action, "reward": reward, "done": done or done2})
            #sum_reward += reward
    ...
    policy_hard = [[action_names[agent.get_legal_action((player_sum, dealer_card, 0, 0, 1), learning=False)] for dealer_card in range(2,12)] for player_sum in range(4, 21)]
    policy_soft = [[action_names[agent.get_legal_action((player_sum, dealer_card, 1, 0, 1), learning=False)] for dealer_card in range(2,12)] for player_sum in range(13, 21)]
